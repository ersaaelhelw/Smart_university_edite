...
public class Wed extends Fragment implements table_adapter.TableClickListener {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        final table_adapter table_adapter = new table_adapter(wed,this);
                            ...
                            modelStu.getTableStudent().observe(getActivity(), new Observer<List<TableEntry>>() {
                                ...
                                public void onChanged(@Nullable List<TableEntry> tableEntries) {
                                    for (int i=0;i<tableEntries.size();i++){
                                        if (tableEntries.get(i).getDay().equals("Wed")){
                                            wed.add(tableEntries.get(i));
                                        }
                                    }
    ...
    public void onListItemClick(int position) {
        ...
        intent.putExtra("docName",wed.get(position).getDoc_name());
        intent.putExtra("subName",wed.get(position).getSub_name());
